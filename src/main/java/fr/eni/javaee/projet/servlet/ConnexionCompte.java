...
public class ConnexionCompte extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		List<Utilisateur> listeDesUtilisateurs = Manager.getInstance().selectCoUtilisateur(choice);

		if (verifierConnexion(choice, password)) {

			HttpSession donneesUtilisateur = request.getSession();
			request.getSession().setAttribute("id",
					UtilisateurManager.findUtilisateurConnecte(choice, listeDesUtilisateurs));
